...
class VisionPipeline:
    ...
    async def pdf_to_images(self, pdf_bytes: bytes, dpi: int = 150) -> list[bytes]:
        ...
        for page_num in range(page_count):
            logger.debug(f"pdf_to_images: rendering page {page_num + 1}/{page_count}")
            page = doc[page_num]
            ...
            logger.debug(f"pdf_to_images: page {page_num + 1} → {len(jpeg_bytes)} bytes JPEG")
    ...
    async def extract_text_from_pdf(
        ...
        logger.debug(f"extract_text_from_pdf: provider={self.llm_handler.provider}, pdf_size={len(pdf_bytes)} bytes")
