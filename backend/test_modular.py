"""Test modular tag-driven workflows against doc 23."""
import asyncio
import sys
import os
...
# Step tags to test (excluding 'process' which runs all steps — tested last)
STEP_TAGS = [
    ...
    # ocr skipped (vision not enabled in config — _step_ocr returns no-op)
    ("process",       MODULAR_TAG_DEFAULTS["modular_tag_process"]),
]
...
async def run_test(step_name: str, trigger_tag_name: str, base_tags: list[int], paperless: PaperlessClient):
    print(f"\n{'='*60}")
    ...
    if result.get("no_modular_tags"):
        print(f"  {RED}FAIL: no modular tags detected{RESET}")
        return
    ...
    for s in result.get("steps", []):
        ...
        print(f"    {status_col}{s['status']:10}{RESET} {s['name']}  ({s['duration_ms']}ms)"
    ...
    final_tag_names = [tag_names.get(tid, str(tid)) for tid in doc_after.get("tags", [])]
    print(f"  tags after: {final_tag_names}")

    # Check trigger tag was removed
    if trigger_id in doc_after.get("tags", []):
        print(f"  {RED}WARNING: trigger tag '{trigger_tag_name}' was NOT removed!{RESET}")
    else:
...
async def main():
    ...
    base_tags = [t for t in original_tags
    ...
    for step_name, trigger_tag in STEP_TAGS:
        ...
        # Reset doc to base_tags between tests
